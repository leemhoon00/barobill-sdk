...
바로빌 전자문서 API TypeScript SDK (비공식)
...
```bash
npm i @barobill-sdk/edoc
```

## 사용법

```typescript
import { EdocClient } from '@barobill-sdk/edoc';

const client = await EdocClient.create({
  certKey: 'your-cert-key',
  corpNum: '1234567890', // 기본 사업자등록번호 (선택)
  test: true,            // true: 테스트 환경 (기본값: false)
});

const result = await client.registAndIssueEDoc({
  // ...
});
```
...
| 분류 | 메서드 |
|------|--------|
| 등록/발행 | `registEDoc`, `registEDocWithHtml`, `issueEDoc`, `registAndIssueEDoc` 등 |
| 수정/삭제 | `updateEDoc`, `updateEDocWithHtml`, `deleteEDoc` |
| 취소 | `cancelEDoc` |
| 조회 | `getEDoc`, `getEDocState`, `getEDocStates`, `getEDocLog` |
| 첨부파일 | `attachFileByFTP`, `attachFilesByFTP`, `deleteAttachFile`, `getAttachedFileList` |
| 문서연결 | `makeDocLinkage`, `removeDocLinkage`, `getLinkedDocs` |
| 전송 | `sendEmail`, `sendSMS`, `sendFax` 등 |
| URL | `getEDocPrintURL`, `getEDocMailURL`, `getBaroBillURL` 등 |
